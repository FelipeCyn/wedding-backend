...
let WeddingService = class WeddingService {
    ...
    create(createWeddingDto) {
        const message = this.weddingRepository.create(createWeddingDto);
        return this.weddingRepository.save(message);
    }
    async update(id, UpdateWeddingDto) {
        const message = await this.weddingRepository.preload(Object.assign({ id: +id }, UpdateWeddingDto));
        if (!message) {
            throw new common_1.NotFoundException(`Message #${id} not found`);
        }
        return this.weddingRepository.save(message);
...
WeddingService = __decorate([
    common_1.Injectable(),
    __param(0, typeorm_1.InjectRepository(wedding_entity_1.Wedding)),
